...
public class ListaEfectividadSupervisor extends Fragment {
    ...
    private void ActualizarFooter() {
        if (variables_publicas.usuario.getTipo().equalsIgnoreCase("Vendedor")){
            ...
            for (HashMap<String, String> item : listaEficiencia) {
                valPor = Double.parseDouble(item.get("Eficiencia").replace(",", "").replace("%", ""));
            }
            lblFooterCantidad.setText("Eficiencia: " + df.format(valPor) + "%");
        }else{
    ...
    private void ActualizarLista() {
        try {
            ...
            DecimalFormatSymbols fmts2 = new DecimalFormatSymbols();
            fmts2.setGroupingSeparator(',');
            df2.setGroupingSize(3);
            df2.setGroupingUsed(true);
            df2.setDecimalFormatSymbols(fmts2);

            for (HashMap<String, String> item : listaEficiencia) {
                double valPor = Double.parseDouble(item.get("Eficiencia").replace(",", "").replace("%", ""));
                double subtotal = Double.parseDouble(item.get("Total").replace("C$", "").replace(",", ""));
                item.put("Eficiencia", df2.format(valPor));
                item.put("Total", df.format(subtotal));
            }
            adapter = new SimpleAdapter(
                    getActivity(), listaEficiencia,
                    R.layout.resumen_cumplimiento_list_item, new String[]{"Dia","Fecha","Ruta","Vendedor","TotalCliente","TotalClienteVenta","TotalClienteNoVenta","TotalClienteNoVisita", "Eficiencia","Total"},
                    new int[]{R.id.lblDetDia, R.id.lblDetFecha, R.id.lblDetRuta, R.id.lblDetVendedor, R.id.lblDetClientes, R.id.lblDetClientesVentas, R.id.lblDetClientesNoVenta, R.id.lblDetClientesNoVisita, R.id.lblDetEficiencia, R.id.lblDetTotal}) {
            };
    ...
    private void GetVisitasService() throws Exception {
        ...
        String urlString="";
        if (variables_publicas.usuario.getTipo().equalsIgnoreCase("Vendedor")){
            urlString = urlVisitasRutasSupervisor + "/" +  Fdesde + "/" +  variables_publicas.usuario.getCodigo() + "/2";
        }else if (variables_publicas.usuario.getTipo().equalsIgnoreCase("Supervisor")){
            urlString = urlVisitasRutasSupervisor + "/" +  Fdesde + "/" +  variables_publicas.usuario.getCodigo() + "/1";
        }else {
            urlString = urlVisitasRutasSupervisor + "/" +  Fdesde + "/" +  variables_publicas.usuario.getCodigo() + "/3";
        }
        try {
